...
В правиле `animation-name` задаётся имя созданной вами анимации `@keyframes`. Правило `animation-duration` указывает сколько секунд анимация будет воспроизводиться. Его нужно указывать в секундах `(3s, 65s, .4s)` или в миллисекундах `(300ms, 1000ms)`.
...
Свойство `animation-delay` определяет задержку до старта анимации, задаётся в секундах или миллисекундах:
...
* `animation-direction: alternate;` — чётные проигрывания анимации будут идти в обратном направлении, а нечетные — в обычном.
* `animation-direction: alternate-reverse;` — чётные проигрывания вашей анимации будут проигрываться в обычном направлении, а нечетные — в обратном.
